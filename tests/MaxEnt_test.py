...
def test_compute_probabilities(ex_const_1, ex_vio_1):
    ...
    output0 = MaxEntUtil.compute_probabilities(ex_const_1.weights[0], ex_vio_1[0])
    assert output0.any() == pred0.any()

    exp1 = np.array([math.exp(-17), math.exp(-7)])
    mar1 = math.exp(-17) + math.exp(-7)
    pred1 = exp1 / mar1

    output1 = MaxEntUtil.compute_probabilities(ex_const_1.weights[1], ex_vio_1[1])
    assert output1.any() == pred1.any()


def test_get_weighted_winner_violations(ex_observed_prob_1, ex_vio_1):
    expected = [np.array([[1 / 3, 2 / 3, 0]]), np.array([[0, 1, 1]])]

    output0 = MaxEntUtil.get_weighted_winner_violations(
        ex_observed_prob_1[0], ex_vio_1[0]
    )
    output1 = MaxEntUtil.get_weighted_winner_violations(
        ex_observed_prob_1[1], ex_vio_1[1]
    )

    assert output0.any() == expected[0].any()
    assert output1.any() == expected[1].any()
...
def test_accessors(ex_const_1):
    ...
    assert ex_const_1.weights.any() == model.cws.any()
